...
const Dashboard = () => {
  ...
  const dashboard = data as DashboardOverviewResponse;

  const formattedDate = new Date(dashboard.meta.generatedAt).toLocaleDateString(
    "en-IN",
    {
      weekday: "long",
      day: "numeric",
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC] p-8 overflow-y-auto custom-scrollbar">
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            MobileShop performance and sales distribution.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          title="Today's Sales"
          value={`₹${dashboard.summary.today.amount.toLocaleString("en-IN")}`}
          percentage={`${dashboard.summary.today.percentage}%`}
          isUp={dashboard.summary.today.percentage >= 0}
          icon={<DollarSign size={20} className="text-cyan-500" />}
        />

        <StatCard
          title="This Week"
          value={`₹${dashboard.summary.week.amount.toLocaleString("en-IN")}`}
          percentage={`${dashboard.summary.week.percentage}%`}
          isUp={dashboard.summary.week.percentage >= 0}
          icon={<Calendar size={20} className="text-purple-500" />}
        />

        <StatCard
          title="This Month"
          value={`₹${dashboard.summary.month.amount.toLocaleString("en-IN")}`}
          percentage={`${dashboard.summary.month.percentage}%`}
          isUp={dashboard.summary.month.percentage >= 0}
          icon={<BarChart3 size={20} className="text-blue-500" />}
        />

        <StatCard
          title="Total Revenue"
          value={`₹${dashboard.summary.totalRevenue.toLocaleString("en-IN")}`}
          isUp={true}
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={dashboard.weeklyTrend}>
              <defs>
